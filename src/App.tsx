...
export default function App() {
  ...
  useEffect(() => {
    const handleHaltEvent = (e: Event) => {
      const customEvent = e as CustomEvent<{ halted: boolean }>;
      if (customEvent.detail?.halted) {
        setAnnouncement('ALERT: WAREFLOW ROBOTICS EMERGENCY HALT ACTIVATED. ALL AMR TRACKS LOCKED.');
      } else {
        setAnnouncement('SYSTEM MESSAGE: WAREFLOW SYSTEM RESUMED. AMR FLEETS OPERATIONAL.');
      }
    };

    window.addEventListener('amr-system-halted', handleHaltEvent);
    return () => {
      window.removeEventListener('amr-system-halted', handleHaltEvent);
    };
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans select-none antialiased">
      ...
      <header className="border-b border-slate-900 bg-slate-950/80 backdrop-blur sticky top-0 z-40 px-6 py-4 flex items-center justify-between">
        ...
        {/* Problem Statement Alignment Badge (Additive Pass) */}
        <div className="hidden lg:flex items-center gap-2 bg-slate-900/60 border border-slate-800 rounded-full px-3 py-1.5 text-[10px] text-slate-400 shadow-inner">
          <span className="font-bold text-sky-400 uppercase tracking-wider text-[9px]">Domain Solutions:</span>
          <span>AMR Telemetry</span>
          <span className="h-1 w-1 bg-slate-700 rounded-full" />
          <span>FIFO Prioritization</span>
          <span className="h-1 w-1 bg-slate-700 rounded-full" />
          <span>Role-Based RLS</span>
          <span className="h-1 w-1 bg-slate-700 rounded-full" />
          <span>Spatial Bins</span>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 p-6 overflow-y-auto min-w-0" key={key}>
          <ErrorBoundary fallbackTitle={`${activeTab.toUpperCase()} Station Crash`}>
      ...
      <div className="sr-only" aria-live="assertive" role="status" aria-atomic="true">
        {announcement}
      </div>
